...
export type EdgeSidecarNodeDto = {
  __typename: string;
  id: string;
  shortcode: string;
  display_url: string;
  is_video: boolean;
  video_url?: string; // Optional as it might be an image
  accessibility_caption?: string; // Optional
  dimensions: DimensionsDto; // Re-use existing DimensionsDto
  // Add other relevant media properties if necessary,
  // mirroring XdtShortcodeMediaDto but perhaps simpler
};

export type EdgeSidecarEdgeDto = {
  node: EdgeSidecarNodeDto;
};

export type EdgeSidecarToChildrenDto = {
  edges: EdgeSidecarEdgeDto[];
};
...
export type XdtShortcodeMediaDto = {
  ...
  upcoming_event: null;
  edge_sidecar_to_children?: EdgeSidecarToChildrenDto; // Added optional property
